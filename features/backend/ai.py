...
def _get_weather(lat: float, lon: float, city_name: str | None = None) -> str | None:
    """Verilen koordinatlar için hava durumunu çeker."""
    url = (
    ...
    try:
        ...
        location = city_name if city_name else "Bulunduğun konum"
        parts = [f"{location} hava durumu: {temp} derece, hissedilen {feels} derece, {desc}"]
...
def _build_system_prompt(include_weather: bool, lat: float | None = None, lon: float | None = None, city_name: str | None = None) -> str:
    ...
    weather_line = ""
    if include_weather:
        # Konum varsa o konumun, yoksa İstanbul'un hava durumunu al
        if lat is not None and lon is not None:
            weather_info = _get_weather(lat, lon, city_name)
        else:
            weather_info = _get_weather(41.0082, 28.9784, "İstanbul")
        if weather_info:
...
def generate_reply(user_text: str, history: list[dict], lat: float | None = None, lon: float | None = None, city_name: str | None = None) -> str:
    ...
    config = types.GenerateContentConfig(
        system_instruction=_build_system_prompt(
            include_weather=_needs_weather(user_text),
            lat=lat,
            lon=lon,
            city_name=city_name,
        ),
